...
3. Expand **Configure a business workflow** to choose UI/background/headless mode, an encrypted credential profile reference, and a daily or HTTP change trigger. Click **Save workflow configuration** to keep the form for later reuse. Save credentials separately, then start a chat and ask the agent to record, test and save the recipe under the same name.
...
Expand **Configure a business workflow** to enter the application type, application/URL, inputs, business steps and expected result. **Save workflow configuration** persists and reloads that definition independently of the connector, provider, credential profile, recipe and schedule. **Review setup in chat** starts the initial plan and supervised recording. **Create autonomous job** persists a daily or HTTP change-triggered scheduled agent task; after the recipe is saved, each run resolves the selected credential profile inside the connector, re-locates semantic targets, captures evidence and requires the stated business postcondition. Headless mode is limited to browser/API workflows; GUI recipes require an available desktop. Web applications should use configured browser tools when available; desktop recipes do not capture browser-tool/API calls.
