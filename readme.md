...
| **Infra RCA**            | Expert remote diagnostics for Linux/Windows servers, network gear, printers, UPS, and databases (SSH/WinRM/SNMP/DB), explicit WinRM transport/authentication, capabilities and quick/full assessments, plus read-only ad-hoc DB queries — proposes fixes but never auto-executes one without your explicit approval, and auto-verifies a fix worked once it runs |
...
2. Wait for **Connected** and the available tool count. Run `get_runtime_status` before recording. On macOS, grant **Accessibility** and **Screen Recording**. Windows requires an unlocked interactive desktop. Linux requires an unlocked X11/XWayland session with `DISPLAY`, `xdotool`, `xrandr`, and one screenshot backend (`maim`, `scrot`, or `gnome-screenshot`). Wayland compositor policy may restrict global input or protected surfaces. Vision-based actions require a configured vision-capable provider.
3. Expand **Configure a business workflow** and use **Process Studio** to add ordered process steps, capture durable reference screenshots, or start a guided agent recording. The first added step defaults to **Open application**. Click **Save workflow draft** to keep the definition; this does not schedule it. Save credentials separately and test the recipe before creating the autonomous job.
...
Expand **Configure a business workflow** to enter the application type, application/URL, inputs, business steps and expected result. **Install autonomous workflow examples** adds guarded drafts for a multi-stage Calculator calculation, daily ERP review, weekly HRMS export, interval queue monitoring and HTTP-triggered reconciliation. The Calculator example performs `12 + 8`, multiplies by `5`, subtracts `25`, and requires an independently read final result of `75`; mismatches and unreadable evidence are failures. Examples never create jobs and must be customized and renamed before readiness can pass. **Process Studio** adds an explicit ordered-step editor, durable reference-screen capture, saved-recipe inventory and guided recording entry point. **Save workflow draft** persists and reloads the definition independently of the connector, provider, credential profile, recipe and schedule. **Save executable recipe** converts the defined open-application/click/type/key/scroll/drag/wait steps into the same recipe format used by guided recording. **Create autonomous job** auto-saves defined steps when necessary, rejects past one-time triggers, shows readiness blockers, and supports one-time, multi-slot daily, selected-day weekly, interval and HTTP change-triggered jobs. Scheduled runs use an approved execution contract that calls the saved recipe instead of returning to the supervised recording plan. After the recipe is tested, each run resolves the selected credential profile inside the connector, re-locates semantic click targets, captures evidence and requires the stated business postcondition. Headless mode is limited to browser/API workflows; GUI recipes require an available desktop. Web applications should use configured browser tools when available; desktop recipes do not capture browser-tool/API calls.
