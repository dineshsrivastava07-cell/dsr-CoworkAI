...
class PluginRegistryStore {
  ...
  constructor() {
    ...
    // Provide a fallback project name outside the Electron process, so the underlying conf init doesn't fail.
  ...
  private resolveStoreCwd(): string {
    try {
      ...
      // Fall back to a default directory in test or non-Electron environments.
    }
