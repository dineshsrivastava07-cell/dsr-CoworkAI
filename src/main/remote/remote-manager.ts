/**
 * Remote Manager
 * Remote-control system manager — integrates the Gateway, Channels, and MessageRouter.
 */
...
export class RemoteManager extends EventEmitter {
  ...
  // Default remote working directory (used for sessions with no cwd specified)
  ...
  /**
   * Sets the default working directory for remote sessions.
   */
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // Set the default remote working directory (prefer config, fall back to the global default)
      ...
      if (tunnelUrl) {
        ...
        log('[RemoteManager] Webhook URL:', `${tunnelUrl}/webhook/websocket`);
      }
  ...
  async handleQuestionRequest(
    ...
    questions.forEach((q, _qIdx) => {
      ...
      if (q.options && q.options.length > 0) {
        ...
        messageText += '\n';
        if (q.multiSelect) {
          messageText += `*(multiple choices allowed, comma-separated, e.g. 1,3)*\n\n`;
        } else {
    ...
    return new Promise((resolve) => {
      this.interactionResolvers.set(toolUseId, (response) => {
        ...
        if (
          ...
          lowerResponse === 'y' ||
          lowerResponse === 'yes' ||
          // Legacy Chinese responses, kept for backward compatibility with older prompts.
          lowerResponse === '允许' ||
          lowerResponse === '是'
        ) {
          resolve({ allow: true });
  ...
  private parseQuestionResponse(
    ...
    // Handle "skip" response (also accepts the legacy Chinese "跳过" for backward compatibility)
    if (
  ...
  /**
   * Sends a remote user's message to the local UI (remote sessions only).
   */
