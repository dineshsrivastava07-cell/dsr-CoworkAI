...
| Windows                        | `winrm`        | TCP 5985/5986 | Supports Basic or NTLM over HTTP/HTTPS; Kerberos uses the Windows V-Coworker host's domain ticket. Select the transport and authentication mode explicitly and keep certificate verification enabled unless an approved internal CA policy says otherwise |
...
2. Enter a unique **Name**, select **Protocol**, and enter **Host** without `https://`, paths or a combined `host:port` value. Use the separate import `port` field for custom ports; IPv6 literals naturally contain colons.
3. For SSH/WinRM/database targets, enter **Username** and **Password**. For SNMP, enter **SNMP community string**. For databases, select **Database engine**. For WinRM, choose **Authentication** (`Auto`, `Basic`, `NTLM`, or `Kerberos`) and **Transport** (`HTTP`/`HTTPS`); use `DOMAIN\\user` or `user@domain` for NTLM/Kerberos. Kerberos uses the signed-in Windows V-Coworker ticket and does not use the password field.
...
The simple form does not expose every backend field. Use a **one-row bulk import** for a custom port, group/site, database name, or SSH private key/passphrase. Leaving the SSH password blank does not automatically select a key from your machine.
...
The connection check shows a failure category, the tested port and corrective steps below **Test**. Expand **Windows IT checks (read-only)** for local PowerShell commands. The chat tool `infra_ping_check` uses the same check and reports failed readiness as a tool error; SNMP targets use a read-only SNMP probe. These checks explain failures but cannot start a service or repair a route on an unreachable computer.
...
3. If the result is connection refused, have the Windows owner run these read-only commands in an administrative PowerShell window on the affected computer:
   ...
   ```powershell
   Get-Service WinRM
   Get-NetTCPConnection -State Listen -LocalPort 5985 -ErrorAction SilentlyContinue
...
6. After IT repairs the failed boundary, repeat the workstation TCP check, then the in-app **Test**, then an authorized read-only diagnostic. Record each result separately. If authentication fails next, verify the selected transport/authentication pair and account format. Basic is for local accounts; NTLM uses `DOMAIN\\user` or UPN; Kerberos uses the Windows V-Coworker domain ticket. Do not enable plaintext Basic traffic or disable firewalls as a troubleshooting shortcut.
