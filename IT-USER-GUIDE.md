...
6. For one authorized pilot computer with no WinRM listener, an administrator can open **Administrator repair** and apply the displayed commands locally or through RDP. They start WinRM, create the standard PowerShell-remoting listener/firewall configuration, and then run `Test-WSMan localhost`. Review the resulting firewall profile and source scope before fleet deployment.
