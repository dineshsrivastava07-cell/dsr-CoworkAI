...
| Windows                        | `winrm`        | TCP 5985     | Current adapter is best-effort and uses Basic authentication over HTTP; it does not expose Kerberos/NTLM or HTTPS configuration. Do not weaken an organizational policy to make this adapter connect; escalate an incompatible environment to engineering |
...
3. For SSH/WinRM/database targets, enter **Username** and **Password**. For SNMP, enter **SNMP community string**. For databases, select **Database engine**.
...
| WinRM policy requires Kerberos/NTLM/HTTPS         | Current adapter does not expose these modes. Escalate the compatibility gap; do not disable organizational controls as a workaround                                                                                                             | INF-07 or mark blocked |
...
4. Interpret the results together:
   ...
   - An HTTPS-only listener commonly uses port 5986. The current V-Coworker adapter does not expose HTTPS transport; changing the target port to 5986 alone is insufficient.
...
6. After IT repairs the failed boundary, repeat the workstation TCP check, then the in-app **Test**, then an authorized read-only diagnostic. Record each result separately. If authentication fails next, investigate the adapter's supported authentication against policy; do not enable plaintext Basic traffic or disable firewalls as a troubleshooting shortcut.
